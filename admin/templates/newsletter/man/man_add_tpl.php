                ...
                <div class="form-group-category row">
                    <?php if(isset($config['newsletter'][$type]['date']) && $config['newsletter'][$type]['date'] == true) { ?>
                        <div class="form-group col-md-4">
                            ...
                            <input type="text" class="form-control text-sm" name="data[date]" id="date" placeholder="Họ tên" value="<?=(!empty($flash->has('date'))) ? $flash->get('date') : @$item['date']?>" required>
                        </div>
                    <?php } ?>
                    <?php if(isset($config['newsletter'][$type]['time']) && $config['newsletter'][$type]['time'] == true) { ?>
                        <div class="form-group col-md-4">
                            <label for="time">Giờ khám:</label>
                            <input type="text" class="form-control text-sm" name="data[time]" id="time" placeholder="Giờ khám" value="<?=(!empty($flash->has('time'))) ? $flash->get('time') : @$item['time']?>" required>
                        </div>
                    <?php } ?>
                    <?php if(isset($config['newsletter'][$type]['service']) && $config['newsletter'][$type]['service'] == true) { ?>
                        <div class="form-group col-md-4">
                            <label for="service">Dịch vụ khám:</label>
                            <input type="text" class="form-control text-sm" name="data[service]" id="service" placeholder="Dịch vụ khám" value="<?=(!empty($flash->has('service'))) ? $flash->get('service') : @$item['service']?>" required>
                        </div>
                    <?php } ?>
                </div>
